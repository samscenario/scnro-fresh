...
-- 7. Create triggers for sensitive data modification logging (INSERT, UPDATE, DELETE only)
...
-- Add triggers to sensitive tables (only for INSERT, UPDATE, DELETE)
CREATE TRIGGER log_campus_bookings_access
  AFTER INSERT OR UPDATE OR DELETE ON public.campus_ticket_bookings
  FOR EACH ROW EXECUTE FUNCTION public.trigger_log_sensitive_access();

CREATE TRIGGER log_lab_applications_access
  AFTER INSERT OR UPDATE OR DELETE ON public.lab_applications
  FOR EACH ROW EXECUTE FUNCTION public.trigger_log_sensitive_access();

CREATE TRIGGER log_signal_invitations_access
  AFTER INSERT OR UPDATE OR DELETE ON public.signal_invitations
  FOR EACH ROW EXECUTE FUNCTION public.trigger_log_sensitive_access();
