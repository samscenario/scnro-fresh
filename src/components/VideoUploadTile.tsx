...
const VideoUploadTile = () => {
  ...
  const loadLatestVideo = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('culture_videos').select('*').eq('category', 'general').order('created_at', {
        ascending: false
      }).limit(1);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('culture-videos').upload(filePath, file);
      if (uploadError) {
      ...
      } = supabase.storage.from('culture-videos').getPublicUrl(filePath);
      console.log('Public URL:', publicUrl);

      // Save to database with 'general' category
      const {
        data: videoData,
        error: dbError
      } = await supabase.from('culture_videos').insert({
        video_url: publicUrl,
        title: file.name,
        file_size: file.size,
  ...
  if (isLoading) {
    return <div className="relative overflow-hidden border border-gray-600 rounded-lg h-full">
  ...
  if (uploadedVideo) {
    ...
    return <div className="relative overflow-hidden border border-acid-green hover:border-electric-blue transition-colors rounded-lg h-full group">
        <video 
          ref={(video) => {
            if (video) {
        ...
        {canEdit && (
          <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="relative">
              ...
              <Button 
                size="sm" 
                className="bg-electric-blue hover:bg-electric-blue/90 text-black"
                disabled={isUploading}
              >
  ...
  return <div className="relative overflow-hidden border border-gray-600 hover:border-electric-blue transition-colors rounded-lg h-full">
      <div className="h-full bg-gradient-to-br from-purple-900/50 to-blue-900/50 flex flex-col items-center justify-center relative p-4">
        <input type="file" accept="video/mp4" onChange={handleFileUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" disabled={isUploading} />
